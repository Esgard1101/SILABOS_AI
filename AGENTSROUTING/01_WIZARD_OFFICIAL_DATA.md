...
- (SPEC-11) Step Fuentes (`Step2_Fuentes.tsx`) is **mandatory**: no "Omitir" button anywhere (footer nor AI modal), and "CONTINUAR" stays `disabled` until `sourceRows.length >= 1` (NotebookLM or Curaduria IA). Reason: progressive unit engines depend on NotebookLM sources for didactics. The gate lives only in Step Fuentes (no double gate in Step8). "Buscar y continuar" in the AI modal counts as bringing sources.
- (SPEC-11) NotebookLM roadmap (`Step2A_NotebookGuide.tsx`) is a **vertical timeline of 4 steps** (Crear cuaderno / Agregar fuentes with Carga manual + Deep Research tabs / Verificar fuentes / Traer fuentes a SIGEISIL), each markable as done (local state, green check, cyan connector). Do not go back to grids/2-row card layouts or color legends. The old "Ajustar chat" step was REMOVED (2026-07-04, owner decision): modern NotebookLM no longer needs chat configuration to return clean JSON — the export prompt (`buildSourcesExportPrompt`) already embeds the strict format. Do not re-add `buildChatSetupPrompt`/chat-setup guidance.
- (SPEC-11) External links pattern: clicking a step/card never auto-`window.open`; the external button (e.g. "Ir a NotebookLM") lives **inside** the modal and always opens with `noopener,noreferrer`.
- (SPEC-11) Guide modals reuse `GlassModal` (`GuideModal`/`GuideContentBody` in `Step2A_NotebookGuide.tsx`); prompts (`buildChatSetupPrompt`, `buildSourcesExportPrompt`, `buildDeepResearchPrompt`) and Cloudinary `NOTEBOOK_VIDEOS.*` are final assets — reuse, never show "Placeholder"/"se reemplazará al cloud" dev texts.
